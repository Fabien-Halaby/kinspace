...
func (r *postgresRepository) Create(ctx context.Context, name, inviteCode string, ownerID int64) (Family, error) {
	...
	result, err := tx.Exec(ctx, `UPDATE users SET family_id=$1 WHERE id=$2 AND family_id IS NULL`, f.ID, ownerID)
	if err != nil { return Family{}, fmt.Errorf("assign family owner: %w", err) }
	if result.RowsAffected() != 1 { return Family{}, ErrAlreadyInFamily }
