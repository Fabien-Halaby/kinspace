...
func (s *Service) Create(ctx context.Context, userID int64, name string) (Family, error) {
	...
	if _, err := s.repo.FindByUserID(ctx, userID); err == nil { return Family{}, ErrAlreadyInFamily } else if !errors.Is(err, ErrFamilyNotFound) { return Family{}, err }
	code, err := generateInviteCode(); if err != nil { return Family{}, fmt.Errorf("generate invite code: %w", err) }
